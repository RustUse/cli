...
use crate::commands::dev::utils::artifacts::{
    GeneratedArtifactReport, inspect_generated_artifacts,
};
use crate::commands::dev::utils::development::{
    DevelopmentSurfaceReport, inspect_development_surface,
};
use crate::commands::dev::utils::documentation::{
    DocumentationSurfaceReport, inspect_documentation_surface,
};
use crate::commands::dev::utils::github::workflows::{
    GitHubWorkflowReport, inspect_github_workflows,
};
use crate::commands::dev::utils::gitlab::{GitLabReport, inspect_gitlab};
use crate::commands::dev::utils::manifest::{
    FacadeManifestReport, analyze_facade_repository_manifests,
};
use crate::commands::dev::utils::nonstandard::{
    NonStandardPathKind, NonStandardPathReport, NonStandardPathRule, inspect_non_standard_paths,
};
use crate::commands::dev::utils::release::{ReleaseSurfaceReport, inspect_release_surface};
use crate::commands::dev::utils::report::{
    markdown_path, resolve_report_path, write_markdown_report, write_presence_table, yes_no,
};
use crate::commands::dev::utils::tooling::{ToolingSurfaceReport, inspect_tooling_surface};
...
#[derive(Debug, Args)]
pub struct DevFacadeReportArgs {
    /// Facade repository root.
    #[arg(default_value = ".", value_name = "ROOT")]
    pub root: PathBuf,

    /// Write the report to this path instead of rustuse-report.md in the facade root.
...
fn write_action_plan(
    ...
    let has_documentation_surface_warnings = documentation_report.status() != "ok";
...
fn write_development_environment(
    ...
    markdown.push_str(&format!(
        "- Present: `{}/{}`\n\n",
        development_report.present_count(),
        development_report.total_count()
    ));
