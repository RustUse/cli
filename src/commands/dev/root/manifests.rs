...
use super::DevRootManifestArgs;
use super::discover::discover_facades;
use super::manifest_fix::{self, ManifestFixOptions};
use super::manifest_flags::manifest_shape_bucket;
use crate::commands::dev::utils::manifest::{
    FacadeManifestReport, ManifestFileReport, ManifestIssue, analyze_manifests,
};
use crate::output::Output;

const TOP_OFFENDER_LIMIT: usize = 15;

pub(crate) fn run(args: DevRootManifestArgs, output: Output) -> Result<()> {
    if args.fix || args.write {
        let mut options = ManifestFixOptions::new(&args.root);
        options.facade = args.facade.clone();
        options.code = args.code.clone();
        options.write = args.write;

        manifest_fix::run(options, output)?;
        return Ok(());
    }
    let root = fs::canonicalize(&args.root).unwrap_or_else(|_| args.root.clone());
    let facades = discover_facades(&root)?;
    let reports = analyze_manifests(&root, &facades)?;

    let manifest_count: usize = reports
        .iter()
        .map(FacadeManifestReport::manifest_count)
        .sum();

    let issue_count: usize = reports.iter().map(FacadeManifestReport::issue_count).sum();

    let error_count: usize = reports.iter().map(FacadeManifestReport::error_count).sum();
    ...
    let invalid_category_count: usize = reports
        .iter()
        .map(FacadeManifestReport::invalid_category_count)
        .sum();

    let has_filters = has_manifest_filters(&args);
    let matching_issue_count = matching_issue_count(&reports, &args);

    output.line(format!(
        "RustUse dev root manifests - root: {}",
        root.display()
    ));
    output.line(format!("facades inspected: {}", reports.len()));
    output.line(format!("manifests inspected: {manifest_count}"));
    output.line(format!(
        "issues: {issue_count} ({error_count} error(s), {warning_count} warning(s))"
    ));
    output.line(format!("invalid category slugs: {invalid_category_count}"));

    if has_filters {
        output.line(format!("matching filtered issues: {matching_issue_count}"));
    }
    ...
    output.line(format!(
        "{:<8} {:<24} {:>9} {:>8} {:>8} {:>10}",
        "Status", "Facade", "Manifests", "Errors", "Warnings", "Bad cats"
    ));
    ...
    write_manifest_issue_summary(&output, &reports);
    write_manifest_shape_summary(&output, &reports);
    write_manifest_top_offenders(&output, &reports);

    if matching_issue_count > 0 {
        output.line("");
        output.line("manifest issues:");

        for facade_report in &reports {
            if !facade_matches_filter(facade_report, &args) {
                continue;
            }
            ...
            for manifest in &facade_report.manifests {
                ...
                if !manifest_matches_filters(manifest, &args) {
                    continue;
                }
                ...
                if let Some(package_name) = &manifest.package_name {
                    output.line(format!("  package: `{package_name}`"));
                }

                output.line(format!("  status: {}", manifest.status()));

                for issue in &manifest.issues {
                    if !issue_matches_filters(issue, &args) {
                        continue;
                    }
    ...
    if has_filters && matching_issue_count == 0 {
        output.line("status: ok, no matching filtered issues");
    } else if error_count > 0 {
        output.line("status: error");
...
fn write_manifest_issue_summary(output: &Output, reports: &[FacadeManifestReport]) {
    let mut summary: BTreeMap<(&'static str, &'static str), usize> = BTreeMap::new();
    ...
    output.line("");
    output.line("manifest issue summary:");
    output.line(format!("{:<8} {:<48} {:>8}", "Severity", "Code", "Count"));
    output.line(format!("{:<8} {:<48} {:>8}", "--------", "----", "-----"));
...
fn write_manifest_shape_summary(output: &Output, reports: &[FacadeManifestReport]) {
    ...
    output.line("");
    output.line("manifest shape summary:");
    output.line(format!("{:<20} {:>8}", "Shape area", "Issues"));
    output.line(format!("{:<20} {:>8}", "----------", "------"));

    for (bucket, count) in rows {
        output.line(format!("{:<20} {:>8}", bucket, count));
    }
}

fn write_manifest_top_offenders(output: &Output, reports: &[FacadeManifestReport]) {
    ...
    for report in reports {
        ...
        rows.push(ManifestTopOffender {
            facade_name: report.facade_name.clone(),
            issue_count,
            error_count: report.error_count(),
            warning_count: report.warning_count(),
            main_shape_area: main_shape_area(report),
        });
    }
    ...
    output.line("");
    output.line(format!(
        "top manifest offenders (top {TOP_OFFENDER_LIMIT}):"
    ));
    output.line(format!(
        "{:<24} {:>8} {:>8} {:>8} {:<20}",
        "Facade", "Issues", "Errors", "Warnings", "Main area"
    ));
    output.line(format!(
        "{:<24} {:>8} {:>8} {:>8} {:<20}",
        "------", "------", "------", "--------", "---------"
    ...
    for row in rows.into_iter().take(TOP_OFFENDER_LIMIT) {
        output.line(format!(
            "{:<24} {:>8} {:>8} {:>8} {:<20}",
            row.facade_name,
            row.issue_count,
...
fn main_shape_area(report: &FacadeManifestReport) -> &'static str {
    let mut buckets: BTreeMap<&'static str, usize> = BTreeMap::new();
...
#[derive(Debug)]
struct ManifestTopOffender {
    facade_name: String,
    issue_count: usize,
    error_count: usize,
    warning_count: usize,
    main_shape_area: &'static str,
}
...
fn has_manifest_filters(args: &DevRootManifestArgs) -> bool {
    args.facade.is_some() || args.code.is_some() || args.kind.is_some() || args.severity.is_some()
}

fn matching_issue_count(reports: &[FacadeManifestReport], args: &DevRootManifestArgs) -> usize {
    reports
        .iter()
        .filter(|report| facade_matches_filter(report, args))
        .flat_map(|report| report.manifests.iter())
        .filter(|manifest| manifest_matches_filters(manifest, args))
        .flat_map(|manifest| manifest.issues.iter())
        .filter(|issue| issue_matches_filters(issue, args))
        .count()
}

fn facade_matches_filter(report: &FacadeManifestReport, args: &DevRootManifestArgs) -> bool {
    option_str_matches(args.facade.as_deref(), report.facade_name.as_str())
}

fn manifest_matches_filters(manifest: &ManifestFileReport, args: &DevRootManifestArgs) -> bool {
    if !option_str_matches(args.kind.as_deref(), manifest.kind.as_str()) {
        return false;
    }

    if args.code.is_none() && args.severity.is_none() {
        return true;
    }
    ...
    manifest
        .issues
        .iter()
        .any(|issue| issue_matches_filters(issue, args))
}

fn issue_matches_filters(issue: &ManifestIssue, args: &DevRootManifestArgs) -> bool {
    option_str_matches(args.code.as_deref(), issue.code)
        && option_str_matches(args.severity.as_deref(), issue.severity.as_str())
}

fn option_str_matches(filter: Option<&str>, value: &str) -> bool {
    match filter {
